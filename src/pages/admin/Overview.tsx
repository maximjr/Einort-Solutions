...
export function Overview() {
  ...
  useEffect(() => {
    ...
    }, (err) => console.error("Error fetching custom projects metrics:", err));
        ...
        <div className="glass-panel geometric-clip border border-white/5 p-6 bg-dark/50 flex flex-col">
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4">
             {recentActivity.length > 0 ? recentActivity.map((log, i) => {
               ...
               if (log.type === 'started_prototype') dotColor = 'bg-blue-400';
               if (log.type === 'completed_prototype') dotColor = 'bg-green-400';
               if (log.type === 'booked_consultation') dotColor = 'bg-premium-gold';
               if (log.type === 'logged_in') dotColor = 'bg-purple-400';
               ...
               <div key={log.id || i} className="flex gap-4 p-4 border border-white/5 rounded-lg bg-white/[0.01] hover:bg-white/[0.03] transition-colors relative overflow-hidden group">
                 ...
                 <div>
                   ...
                   <p className="font-mono text-[9px] text-silver-metallic tracking-wider md:mt-1 truncate max-w-[160px]">{log.desc}</p>
                 </div>
                 <span className="ml-auto font-mono text-[9px] text-silver-metallic whitespace-nowrap">{formatDistanceToNow(log.time, { addSuffix: true })}</span>
               </div>
             )}) : (
