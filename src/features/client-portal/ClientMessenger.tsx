...
export function ClientMessenger({ userId, userEmail, userName, onClose }: ClientMessengerProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent border-none overflow-hidden relative">
      ...
      <div className="px-4 py-3.5 md:px-5 md:py-4 bg-[#0a0f18]/80 border-b border-white/[0.05] flex items-center justify-between relative z-15 backdrop-blur-3xl">
        <div className="flex items-center gap-3.5">
          <div className="relative">
            <div className={`w-10 h-10 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 hover:from-primary/30 hover:to-primary/10 border border-primary/20 flex items-center justify-center font-medium text-sm text-primary uppercase shadow-inner transition-colors`}>
              SA
            </div>
            {/* Realtime Admin Position Glow */}
            <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#0a0f18] shadow-[0_0_8px_rgba(52,211,153,0.4)] ${adminOnline ? "bg-emerald-400" : "bg-slate-600 shadow-none"}`}></span>
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-[13px] font-semibold tracking-wide text-white">Super Admin</h4>
              <span className={`text-[9px] px-2 py-0.5 rounded-full uppercase font-bold tracking-widest ${adminOnline ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-slate-400"}`}>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6 bg-gradient-to-b from-transparent to-black/10">
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-400">
            ...
            <p className="text-[11px] font-medium tracking-wide">Connecting secure tunnel...</p>
          </div>
        ) : messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-5">
            ...
            <div className="space-y-2">
              <h5 className="text-[15px] font-semibold text-white tracking-wide">Communication Pipeline Ready</h5>
              <p className="text-slate-400 font-light text-[13px] leading-relaxed max-w-sm mx-auto">
                Your dedicated architect is ready to assist. Send deliverables, request audits, or start a new priority thread.
              </p>
            </div>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className={`max-w-[75%] flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                    <div className={`px-4 py-3 text-[13.5px] font-light tracking-[0.01em] shadow-[0_4px_24px_rgba(0,0,0,0.2)] transition-all ${
                      isMe 
                      ...
                      {ms.attachments && ms.attachments.length > 0 && (
                        <div className={`mt-3 pt-3 border-t space-y-2 ${isMe ? "border-white/20" : "border-white/10"}`}>
                          {ms.attachments.map((attach, aIdx) => (
                            <a 
                              ...
                              className={`flex items-center gap-2 text-[12px] font-medium hover:underline transition-colors ${
                                isMe ? "text-white/90 hover:text-white" : "text-primary/90 hover:text-primary"
                              }`}
                            >
      ...
      {/* Message input panel */}
      <form onSubmit={handleSendMessage} className="p-3 md:p-4 bg-[#0a0f18]/90 border-t border-white/[0.05] relative z-10 backdrop-blur-3xl pb-[max(1rem,env(safe-area-inset-bottom))]">
        {mockAttachment && (
        ...
        <div className="flex items-end gap-2.5">
          ...
          <div className="relative shrink-0 mb-1">
            ...
            <AnimatePresence>
              {showAttachMock && (
                ...
                >
                  <p className="text-[10px] uppercase font-semibold text-slate-400 px-3 py-2 border-b border-white/5 mb-1.5">Project Assets</p>
                  <button 
                    type="button"
                  ...
                  >
                    <Paperclip size={12} className="text-primary" /> Blueprint Document
                  </button>
                  <button 
                    type="button"
                    ...
                    className="w-full text-left font-medium text-[12px] text-slate-200 hover:text-white hover:bg-white/5 py-2 px-3 rounded-lg transition-colors cursor-pointer flex items-center gap-2"
                  >
                    <Paperclip size={12} className="text-primary" /> Deliverable Audit
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={sending ? "Sending..." : "Message Super Admin..."}
              disabled={sending}
              rows={1}
